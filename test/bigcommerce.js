...
describe("BigCommerce", () => {
	...
	describe("#authorize", () => {
		beforeEach(() => {
			self.runStub = self.sandbox.stub(Request.prototype, "run").returns(Promise.resolve({ test: true }));
		});

		const query = { code: "", scope: "", context: "" };

		it("should return an object", () => {
			return bc.authorize(query).then((data) => data.should.not.be.null);
		});
	...
	describe("#request", () => {
		beforeEach(() => {
			self.requestStub = self.sandbox.stub(Request.prototype, "run").returns(Promise.resolve({ text: "" }));
		});

		it("should make a call to the request object", () => {
			return bc.request("get", "/foo").then(() => sinon.assert.calledOnce(self.requestStub));
		});
