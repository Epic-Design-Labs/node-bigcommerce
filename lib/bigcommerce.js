...
class BigCommerce {
	constructor(config) {
		if (!config) {
			...
			const { siteUrl = null, clientId = null, secret = null, accessToken = null, storeHash = null, headers = {}, callback = null, logLevel = "info", responseType = "json" } = config;
	...
	async verify(signedRequest) {
		...
		this.logger.info("Expected Signature: " + expected);
