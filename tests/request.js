...
describe("Request", () => {
	...
	context("given a 429 status code", () => {
		...
		it("should retry the request", () => {
			return request.run("post", "/orders").then(() => self.ordersCall.isDone().should.equal(true));
		});
	...
	context('if "error" are found in the response JSON', () => {
		beforeEach(() => {
			nock("https://api.bigcommerce.com").post("/orders").reply(200, { error: "An error has occurred." });
		});
	...
	context("if json is not returned", () => {
		...
		it("should return the raw response", () => {
			return request.run("post", "/orders", {}).then((res) => res.should.equal("<xml></xml>"));
		});

		it("should attach the response if the JSON cannot be parsed", () => {
			return request.run("post", "/customers", {}).catch((err) => err.should.have.property("responseBody"));
		});
	});

	context("timeout", () => {
		beforeEach(() => {
			nock("https://api.bigcommerce.com").post("/orders").replyWithError("ECONNRESET");
		});
	...
	it("should attach a keep-alive HTTPS agent", () => {
		nock("https://api.bigcommerce.com").post("/orders").reply(200, { order: true });
		...
		return request.run("post", "/orders").then((res) => res.should.be.a("object"));
	});

	it("should return a JSON object on success", () => {
		nock("https://api.bigcommerce.com").post("/orders").reply(200, { order: true });

		return request.run("post", "/orders").then((res) => {
